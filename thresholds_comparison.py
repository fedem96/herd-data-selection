...
for i in range(len(models_names)):
    ...
    if model_name not in evaluations:
        ...
        if particles == "electrons":
            if avg_pooled == 0:
                if model_name.startswith("baseline"):
                    metric_reference = 0.883
                    metric_reference_sqrt = 0.995
                elif model_name.startswith("perceptron"):
                    metric_reference = 0.994
                elif model_name.startswith("conv2d"):
                    metric_reference = 0.998
                elif model_name.startswith("conv3d"):
                    metric_reference = 1.000
                else:
                    print("unknown model")
                    exit(7)
            elif avg_pooled == 1:
                if model_name.startswith("baseline"):
                    metric_reference = 0.883
                    metric_reference_sqrt = 0.994
                elif model_name.startswith("perceptron"):
                    metric_reference = 0.992
                elif model_name.startswith("conv2d"):
                    metric_reference = 0.999
                elif model_name.startswith("conv3d"):
                    metric_reference = 0.999
                else:
                    print("unknown model")
                ...
                print("missing data for double average pooling")
                exit(6)
        else:  # protons
            if avg_pooled == 0:
                if model_name.startswith("baseline"):
                    metric_reference = 0.411
                    metric_reference_sqrt = 0.470
                elif model_name.startswith("perceptron"):
                    metric_reference = 0.529
                elif model_name.startswith("conv2d"):
                    metric_reference = 0.980
                elif model_name.startswith("conv3d"):
                    metric_reference = 0.987
                else:
                    print("unknown model")
                    exit(7)
            elif avg_pooled == 1:
                if model_name.startswith("baseline"):
                    metric_reference = 0.411
                    metric_reference_sqrt = 0.472
                elif model_name.startswith("perceptron"):
                    metric_reference = 0.540
                elif model_name.startswith("conv2d"):
                    metric_reference = 0.972
                elif model_name.startswith("conv3d"):
                    metric_reference = 0.973
                else:
                    print("unknown model")
                    exit(7)
            else:
        ...
        evaluations[model_name] = [metric_reference, {}]
        if model_name.startswith("baseline"):
            evaluations[model_name+"_sqrt"] = [metric_reference_sqrt, {}]
    ...
    if model_name.startswith("baseline"):
        print("plotting dotted line for model " + model_name + "_sqrt, val = " + str(evaluations[model_name+"_sqrt"][0]))
        plt.semilogx(thresholds, [evaluations[model_name+"_sqrt"][0] for k in range(len(thresholds))], 'c--', basex=2, label=model_name+"_sqrt")
...
#             exit(6)
#
#     plt.semilogx(thresholds, [metric_reference for k in range(len(thresholds))], color + '--', basex=2)
#     print("plotting dotted line for model " + model_name + ", val = " + str(metric_reference))
#     if model_name.startswith("baseline"):
#         plt.semilogx(thresholds, [metric_reference_sqrt for k in range(len(thresholds))], 'c--', basex=2)
#         print("plotting dotted line for model " + model_name + "_sqrt, val = " + str(metric_reference_sqrt))
